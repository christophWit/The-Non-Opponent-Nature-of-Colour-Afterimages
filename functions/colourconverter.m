function colours2 = colourconverter(colours0, col_space, dim, wp_xyY, conesens, cmf, mon_xyY, mon_ldt, mon_oog, mon_kwY, mon_rgbmax, bgY, msg)
...
%              coordinates of DKL axis. For DKL, CIELUV, and CIELAB, polar
%              coordinates can be specified. These are entered in the
%              format: [Lightness, Hue in degree, Chroma as radius].
% - col_space = Colour space of your colours0; default DKL.
% - wp_xyY  = White-point for CIELUV and CIELAB conversions.
% - conesens = cone sensitivities used to calculate LMS and DKL; the most
%   precise are Stockman-Sharpe cone fundamentanls (= default).
% - cmf     = colour matching functions used to convert from XYZ to LMS;
%   CIELUV and CIELAB are based on the older CIE1931 cmfs (= default).
...
% - bgY     = Background luminance; necessary for DKL when background (for
%             DKL) and assumed white-point (CIELUV, CIELAB) differ in 
%             luminance.
...
% 2019.12.19 changed 2D-output from dataset to table format [cw]
% 2020.04.01 corrected uint8 bug by converting colours0 to double [cw]
% 2020.07.07 added oog for sRGB default monitor setting [cw]
% 2021.05.15 changed dkl_pol column order + azi in deg to match Luv_pol [cw]
% 2023.01.23 fixed DKL bug by commenting if-condition [cw]
% 2025.07.16 added lms (Stockmann-Sharpe to cie1931) input [cw]
% 2025.07.20 added mod(..., 360) for polar output [cw]
        ...
        if nargin < 11
            ...
            if nargin < 10
                ...
                if nargin < 9
                    ...
                    if nargin < 8
                        [tmp, mon_ldt, mon_oog] = srgb; % note: writes over mon_oog
                        if nargin < 7
                            mon_xyY = tmp;
                            if nargin < 6
                                cmf = '1931';
                                if nargin < 5
                                    conesens = 'stockmansharpe';
                                    if nargin < 4
                                        wp_xyY = [];
                                        if nargin < 3
                                            dim = 2;
...
colours0 = double(colours0); % to avoid problems with uint8

% CONVERT 3 DIMENSIONS TO 2 -----------------------------------------------
if dim == 3
    [sz1, sz2, ~] = size(colours0); 
    colours0 = reshape(colours0, [], 3);
end
...
if isempty(bgY)
    bgY = 'halfwp';
    warning('Background luminance has been set to half white-point luminance');
end
...
if ischar(bgY)
    switch bgY
        ...
        case 'halfwp'
            bgY = wp_xyY(3)/2;
        case 'mon'
            xyY = XYZ2xyY(rgb2XYZ([1,1,1], mon_xyY,2, mon_kwY));
            bgY = xyY(3); 
        case 'halfmon'
            xyY = XYZ2xyY(rgb2XYZ([1,1,1], mon_xyY,2, mon_kwY));
            bgY = xyY(3)/2; 
...
% MONITOR & BACKGROUND LMS FOR DKL CONVERSION (Fit to monitor gamut) ------
mon_lms     = XYZ2lms(xyY2XYZ(mon_xyY), conesens, cmf);
bg_xyY      = [wp_xyY(1:2), bgY];
bg_lms      = XYZ2lms(xyY2XYZ(bg_xyY), conesens, cmf);

% CONVERT POLAR INPUT TO CARTESIAN COORDINATES ----------------------------
% Hue (second coordinate) is converted to radians, 
% then, hue and chroma (third coordinate) are converted to Cartesian coordinates.
switch lower(col_space)
    case {'dkl_pol', 'luv_pol', 'lab_pol'}
        [x,y] = pol2cart(deg2rad(colours0(:,2)),colours0(:,3));
        colours0 = [colours0(:,1), x,y];
end

% INITIAL CONVERSION OF INPUT COLOURS -------------------------------------
switch lower(col_space)
    case 'rgb'
        rgb = colours0; 
        rgbl = gammainvcorrector(rgb, mon_ldt, 2, mon_rgbmax);
    case 'rgbl'
        rgbl = colours0; 
    case 'xyy'
        xyY = colours0;
    ...
    case {'dkl', 'dkl_cart', 'dkl_pol'}
        dkl = colours0;
        lms = dkl2lms(dkl, bg_lms, mon_lms);
        XYZ = lms2XYZ(lms, conesens, cmf);
        if msg
            switch lower(conesens)
                case 'judd'
                otherwise
                    fprintf('WARNING: Most commnon DKL algorithms involve Judd corrected xyY\n');
            end
        end
    case {'luv','luv_pol'}
        Luv = colours0;
        [xyY, XYZ] = Luv2xyY(colours0, wp_xyY, 'xyY', 2);
    case {'lab','lab_pol'}
        Lab = colours0;
        [xyY, XYZ] = Lab2xyY(colours0, wp_xyY, 2);
    case 'hsv'
        hsv = colours0;
    ...
    case 'lms'
        ...
        if msg
            switch lower(conesens)
                case '1931'
                otherwise
                    fprintf('WARNING: CIELUV and CIELAB are gauged to CIE1931 colour matching functions\n');
            end
        end
end
...
% CONVERSION TO XYZ -------------------------------------------------------
if isempty(XYZ)
    tmp = rgbl/mon_rgbmax; % To avoid warning.
    XYZ = rgb2XYZ(tmp, mon_xyY(1:3,:), 2, mon_kwY);
end
...
% kY-Correction -----------------------------------------------------------
if ~isempty(mon_kwY)
    r_xyY = black_luminance_corrector(xyY, 2, mon_kwY, msg);
else
    r_xyY = xyY;
    if msg
...
[r_rgbl, gamut0, gamut1]  = gamutchecker(rgbl, mon_oog, mon_rgbmax, 2, msg);
oog = gamut0+gamut1;

% GAMMA -------------------------------------------------------------------
% Gamma correction (Translate linear values into gamma distributed ones)
if isempty(rgb)
    rgb = gammacorrector(r_rgbl, mon_ldt, 2);
end
...
% LUV ---------------------------------------------------------------------
if isempty(Luv)
    Luv = xyY2Luv(xyY, wp_xyY, 2);
end

% LAB ---------------------------------------------------------------------
if isempty(Lab)
    Lab = xyY2Lab(xyY, wp_xyY, 2);
end

% LMS ---------------------------------------------------------------------
if isempty(lms)
    lms = XYZ2lms(XYZ, conesens, cmf);
end
...
dkl_pol(:,1) = mod(rad2deg(azi1),360);
Luv_pol(:,1) = mod(rad2deg(azi2),360);
Lab_pol(:,1) = mod(rad2deg(azi3),360);

%% OUTPUT

if dim == 2
    colours2 = table(rgb, oog, rgbl, hsv, XYZ, xyY, lms, mlb, dkl, dkl_pol, Luv, Luv_pol, Lab, Lab_pol);
elseif dim == 3
    colours2 = struct(...
        'rgb', rgb,...
        'oog', oog,...
        ...
        'Lab', Lab,...
        'Lab_pol', Lab_pol);
    fldnms = fieldnames(colours2);
    for fld = 1:numel(fldnms)
        dim = size(colours2.(fldnms{fld}),2);
        colours2.(fldnms{fld}) = reshape(colours2.(fldnms{fld}), sz1, sz2, dim);
    end
end
%% ***************************** SUBFUNCTIONS *****************************
